extension Theme {
    var alertConfigurationStyle: AlertConfiguration {
        ...
        let leaveQueue = ConfirmationAlertConfiguration(title: Alert.LeaveQueue.title,
                                                        message: Alert.LeaveQueue.message,
                                                        negativeTitle: Alert.Action.no,
                                                        positiveTitle: Alert.Action.yes)
        let endEngagement = ConfirmationAlertConfiguration(title: Alert.EndEngagement.title,
                                                           message: Alert.EndEngagement.message,
                                                           negativeTitle: Alert.Action.no,
                                                           positiveTitle: Alert.Action.yes)
        let operatorsUnavailable = MessageAlertConfiguration(title: Alert.OperatorsUnavailable.title,
        ...
        let phoneAction = MediaUpgradeActionStyle(title: Alert.MediaUpgrade.Phone.title,
                                                  ...
                                                  iconColor: color.primary)
        let mediaUpgrade = MediaUpgradeAlertConfiguration(title: Alert.MediaUpgrade.title,
                                                          audioUpgradeAction: audioAction,
                                                          phoneUpgradeAction: phoneAction)
        let audioUpgrade = AudioUpgradeAlertConfiguration(title: Alert.AudioUpgrade.title,
                                                          titleImage: Asset.upgradeAudio.image,
                                                          decline: Alert.Action.decline,
                                                          accept: Alert.Action.accept)
        let microphoneSettings = SettingsAlertConfiguration(title: Alert.MicrophonePermission.title,
                                                            message: Alert.MicrophonePermission.message,
                                                            settingsTitle: Alert.Action.settings,
                                                            cancelTitle: Alert.Action.cancel)
        let unexpected = MessageAlertConfiguration(title: Alert.Unexpected.title,
                                                   message: Alert.Unexpected.message)
        let api = MessageAlertConfiguration(title: Alert.ApiError.title,
                                            message: Alert.ApiError.message)

        return AlertConfiguration(leaveQueue: leaveQueue,
