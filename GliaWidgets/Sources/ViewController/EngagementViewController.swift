...
class EngagementViewController: UIViewController, AlertPresenter {
    ...
    func bind(engagementViewModel: CommonEngagementModel) {
        ...
        view.header.props.endButton.tap = .init { [weak self] in self?.viewModel.event(.closeTapped) }
        view.header.props.endScreenshareButton.tap = .init { [weak self] in self?.viewModel.event(.endScreenSharingTapped) }
        view.header.props.backButton.tap = .init { [weak self] in self?.viewModel.event(.backTapped) }
        view.header.props.closeButton.tap = .init { [weak self] in self?.viewModel.event(.closeTapped) }
