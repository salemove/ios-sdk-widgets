...
extension CoreSdkClient.Logger {
    init(_ logging: CoreSdkClient.Logging) {
        self.debugClosure = { logging.debug($0(), file: $1, function: $2, line: $3) }
        self.infoClosure = { logging.info($0(), file: $1, function: $2, line: $3) }
        self.errorClosure = { logging.error($0(), file: $1, function: $2, line: $3) }
        self.warningClosure = { logging.warning($0(), file: $1, function: $2, line: $3) }
        self.localLoggerClosure = { logging.localLogger.map(Self.init) }
        self.remoteLoggerClosure = { logging.remoteLogger.map(Self.init) }
        self.oneTimeClosure = { Self(logging.oneTime) }
