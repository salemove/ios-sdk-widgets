...
extension CallVisualizer.VideoCallView {
    final class ConnectOperatorView: BaseView {
        ...
        var renderedAnimating: Bool = false {
            didSet {
...
extension CallVisualizer.VideoCallView.ConnectOperatorView {
    struct Props: Equatable {
        ...
        init(
            style: ConnectOperatorStyle = .init(operatorImage: .mock(), animationColor: .white),
            size: SizeObject = .init(size: .normal, animated: false),
            operatorAnimate: Bool = false,
            userImageViewProps: CallVisualizer.VideoCallView.UserImageView.Props = .init()

        ) {
            self.style = style
            self.size = size
            self.operatorAnimate = operatorAnimate
            self.userImageViewProps = userImageViewProps
        }
    }
}
